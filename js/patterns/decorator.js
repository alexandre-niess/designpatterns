...
class HighPriorityDecorator extends TaskDecorator {
  ...
  getHtmlRepresentation() {
    ...
    html = html.replace(
      ...
      "list-group-item task-item high-priority-task"
    );
    // Adicionando badge de prioridade alta
    html = html.replace(
      '<span class="badge',
      '<span class="badge bg-danger me-2">PRIORITÁRIO</span><span class="badge'
    );
    return html;
  }
}
  ...
  getHtmlRepresentation() {
    ...
    html = html.replace(
      ...
      `<span class="badge label-${
        this.color
      } me-2">${this.getColorName()}</span><span class="badge`
    );
    return html;
  }
...
class DueDateDecorator extends TaskDecorator {
  ...
  getHtmlRepresentation() {
    ...
    html = html.replace(
      ...
      `Criada em: ${this.task
        .getCreatedAt()
        .toLocaleString()} | <strong class="due-date">Vencimento: ${formattedDate}</strong>`
    );
    return html;
  }
}
