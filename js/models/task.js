    ...
    return `
            <div class="list-group-item task-item" id="task-${this.id}">
                ...
                <div class="d-flex justify-content-between align-items-center mt-2">
                    ...
                    <div class="btn-group">
                        <button class="btn btn-sm btn-outline-primary status-btn" data-task-id="${
                          this.id
                        }" data-status="em_andamento">Em Andamento</button>
                        <button class="btn btn-sm btn-outline-success status-btn" data-task-id="${
                          this.id
                        }" data-status="concluida">Concluída</button>
                        <button class="btn btn-sm btn-outline-danger delete-btn" data-task-id="${
                          this.id
                        }">Excluir</button>
                    </div>
                </div>
...
class PersonalTask extends Task {
  ...
  getHtmlRepresentation() {
    ...
    return baseHtml.replace(
      ...
      "list-group-item task-item task-personal"
    );
  }
}
...
class WorkTask extends Task {
  ...
  getHtmlRepresentation() {
    ...
    return baseHtml.replace(
      "list-group-item task-item",
      "list-group-item task-item task-work"
    );
  }
}
...
class StudyTask extends Task {
  ...
  getHtmlRepresentation() {
    ...
    return baseHtml.replace(
      "list-group-item task-item",
      "list-group-item task-item task-study"
    );
  }
}
...
class UrgentTask extends Task {
  ...
  getDetails() {
    return `[URGENT] ${super.getDetails()}`;
  }
}
